...
# Question 2
var_seconds <- 250
minutes <- round(var_seconds/60)
seconds <- var_seconds %% 60
str_minutes <- ""
if(minutes == 1){
  str_minutes <- paste(minutes, "minute")
}else{
  str_minutes <- paste(minutes, "minutes")
}
str_seconds <- ""
if(seconds == 1){
  str_seconds <- paste(seconds, "second")
}else{
  str_seconds <- paste(seconds, "seconds")
}
cat(var_seconds, " seconds is ", str_minutes, " and ", str_seconds)
```
...
for(i in 1: num){
  if(i*i > num){
    cat(i, " squared is ", i*i, ", and is the first number whose square exceeds ",num, ".", sep = "")
    break
  }
}
```
