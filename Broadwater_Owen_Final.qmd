...
count <- 0
while(sum < 30){
  sum <- sum + sample(1:10, 1)
  count <- count + 1
}
sprintf("You generated %s numbers that sum to %s", count, sum)
```
...
bmi_calc <- function(height, weight){
  ...
  return(c(bmi, designation))
}
bmi <- bmi_calc(67, 180)
paste("Your BMI is",bmi[2],"with a value of",bmi[1], sep = " ")
```
...
```{r}
vec_sums <- lapply(vec_list, function(x) sum(x) %% 2 == 0)
print(vec_sums)
```
...
```{r}
heaviest_weight <- max(fruit_info$weight)
print(fruit_info[fruit_info$weight == heaviest_weight, ]$fruit)
```
...
```{r}
for(col in names(fruit_info)){
  for(i in 1:nrow(fruit_info)){
    if(fruit_info$weight[i] > 100){
      cat(fruit_info$fruit[i], "is heavy.\n")
    }else{
      cat(fruit_info$fruit[i], "is light.\n")
    }
  }
}
```
