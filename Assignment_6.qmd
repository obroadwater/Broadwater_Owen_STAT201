...
for(i in 1:length(bears)){
  if(spread < bears[i]-opponent[i]){
    spread <- bears[i]-opponent[i]
    game <- i
  }
}
paste("The largest win spread is",spread,"when the Bears scored",bears[game], "points and the Opponent scored",opponent[game], "points")
```
...
```{r}
game_score <- cbind(bears = bears, opponent = opponent)
print(game_score)
```
...
```{r}
game_score <- rbind(game_score, cbind(c(37,9),c(17,17)))
print(game_score)
```
...
```{r}
paste(which(game_score[,'win'] == TRUE)[1]-1, "games lost before first win")
```
...
area_rectangle <- function(length = 48, width = 48){
  return(c(length*width,length*width/144))
}
rec_area <- area_rectangle(102, 60)
paste("The area in square inches is",rec_area[1])
paste("The area in square feet is",rec_area[2])
```
...
```{r}
most_letters <- function(s){
  num_letters <- nchar(s)
  longest <- which(nchar(s) == max(num_letters))
  return(s[longest])
}
...
```{r}
is_in <- function(val, vec){
  return(any(vec == val))
}
if(is_in(value, check)){
